...
//		public var first:Boolean=false;
		...
		private function getRoleTip():void //获得角色说明
		{
...
//				roletiptxt+=GL.taskData.get(GL.roleData.get(rid).st).n;
				...
				//test sort以下为按技能顺序显示技能说明的测试，如果成功，可替代上面注释掉的代码。
				roletiptxt="";
				var arr:Array=skillHash.keys();
				function sortNumber(a,b):int{
					return a - b;
				}
				arr.sort(sortNumber);
				for(var i:int in arr){
					roletiptxt+=StringUtils.getColorString(skillHash.get(arr[i]).name,Trans.getSkillColor(skillHash.get(arr[i]).color))+"\n";
					roletiptxt+=skillHash.get(arr[i]).desc+"\n";
				}
				roletiptxt+="────────────────";
				roletiptxt+=StringUtils.getColorString("酱油任务",0x00ff00)+"\n";
				roletiptxt+=GL.taskData.get(GL.roleData.get(rid).st).n;
		...
		public function clearRoleTip():void{
			roletiptxt = null;
		}
		...
		public function initSkillByRid(rid:int):void{ //初始化技能（包括技能按钮）
			if(rid>0){
