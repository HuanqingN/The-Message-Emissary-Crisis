		...
		protected function onmouseclick(event:MouseEvent):void
		{
			...
			if(selectType==0){
				...
				if(shash.size()>0){
					...
					if(selectNum>0){
						if(selectType == 70)param.type=event.currentTarget.name=="hand"?0:1;//纳税。hand=放置。如果没选牌，点击放置则视为放置，点击交出则视为交出。 //这句可以考虑注释掉，因为上面有一句重复的。
					}else{
						if(selectType == 71){
							param.type==event.currentTarget.name=="hand"?0:1; //猎杀。hand=放置,info=不放置
						}else param.type=0;
					}
				}
			}
